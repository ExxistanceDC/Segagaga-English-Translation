...
>What I discovered is that the game actually copies the entirety of the BIOS font into RAM pretty near the location where the game executable ends! So, I disabled the assembly code that performed this copy, then also disabled the code that null’d out that portion of RAM before doing the copy. I then appended the BIOS font to the end of the game executable, and just like that, the font data was precisely where the game expected it to be, and it could be modified! This came in handy a number of times as font glyphs needed tweaking for optimal appearance.</ul>
